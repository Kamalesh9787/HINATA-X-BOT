...
async def next_page(bot, query):
    ...
    if SHORT_URL and SHORT_API:          
        if settings["button"]:
            btn = [[InlineKeyboardButton(text=f"♦️[{get_size(file.file_size)}]-🔷-{file.file_name}", url=await get_shortlink(f"https://telegram.dog/{temp.U_NAME}?start=files_{file.file_id}"))] for file in files ]
        else:
        ...
        if settings["button"]:
            btn = [[InlineKeyboardButton(text=f"♦️[{get_size(file.file_size)}]-🔷-{file.file_name}", callback_data=f'files#{nxreq}#{file.file_id}')] for file in files ]
        else:
    ...
    if n_offset == 0:
        btn.append(
            [InlineKeyboardButton("⏪ BΛ匚K", callback_data=f"next_{req}_{key}_{off_set}"),
             InlineKeyboardButton(f"📒 ᎮΛGΣ {math.ceil(int(offset) / 10) + 1} / {math.ceil(total / 10)}", callback_data="pages")]
        )
...
async def auto_filter(client, msg, spoll=False):
    ...
    if SHORT_URL and SHORT_API:          
        if settings["button"]:
            btn = [[InlineKeyboardButton(text=f"♦️[{get_size(file.file_size)}]-🔷-{file.file_name}", url=await get_shortlink(f"https://telegram.dog/{temp.U_NAME}?start=pre_{file.file_id}"))] for file in files ]
        else:
        ...
        if settings["button"]:
            btn = [[InlineKeyboardButton(text=f"♦️[{get_size(file.file_size)}]-🔷-{file.file_name}", callback_data=f'{pre}#{req}#{file.file_id}')] for file in files ]
        else:
    ...
    if offset != "":
        ...
        req = message.from_user.id if message.from_user else 0
        btn.append(
            [InlineKeyboardButton(text=f"📒 ᎮΛGΣ 1/{math.ceil(int(total_results) / 6)}", callback_data="pages"),
             InlineKeyboardButton(text="ПΣXƬ ⏩", callback_data=f"next_{req}_{key}_{offset}")]
        )
    else:
        btn.append(
            [InlineKeyboardButton(text="📒 ᎮΛGΣ 1/1", callback_data="pages")]
        )
...
async def advantage_spell_chok(msg):
    ...
    if not g_s:
        ...
        await msg.reply("I Cᴏᴜʟᴅɴ'ᴛ Fɪɴᴅ Aɴʏ Mᴏᴠɪᴇ Iɴ Tʜᴀᴛ Nᴀᴍᴇ", quote=True)
